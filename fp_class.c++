...
int fprint_read(ifstream &in1, unsigned char *(*fp1),
  ...
  if      (strcmp(type_fprint, "generic") == 0) 
    fprint_read_generic(in1, fp1, name1, no1, len_fp1);
  else if (strcmp(type_fprint, "unity") == 0)
    fprint_read_unity(in1, fp1, name1, no1, len_fp1);
  else
...
int fprint_read_write_generic(ifstream &in1, ofstream &out1, int *fp_clstr_idx,
                      int len_fp1) {
  char buffer1[MAX_LINE_SIZE];
  char buffer2[MAX_LINE_SIZE];
  int i, j, k;
...
int init_byte_2_bits(){
  ...
  for (c1=0,i=0; i<256; c1++,i++)
    for (c2=0,j=0; j<256; c2++,j++)
      bits_Nab[c1][c2] = byte_2_bits[c1 & c2];
  return 1;
}
...
int  fprint_sort_uniq(int *bit1, unsigned char *(*fp1), int len_fp1, 
                      int *idx1, int no1) {
  int i, j, k;
  for (i=0; i<no1; i++) idx1[i] = i;
...
// sort only idx, not a
int quick_fprint_sort_uniq(int *a, unsigned char *(*fp1), int len_fp1,
                           int *idx,  int lo0, int hi0 ) {
  int lo = lo0;
  int hi = hi0;
  int mid;
